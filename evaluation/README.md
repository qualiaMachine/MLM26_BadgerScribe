...
Single source of truth for scoring. There is no leaderboard and no automated scoring — you run this code against the released evaluation set yourself and report the numbers in your writeup; organizers re-run the same code when verifying writeups in contention for recognition.
...
- **[`score_local.py`](score_local.py)** — CLI: `python score_local.py --solution eval/solution.csv --submission my_predictions.csv`. Prints per-category CER, diagnostic WER, and the overall macro CER — the values you report in your writeup.
