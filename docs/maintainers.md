...
- **All UW ground truth goes into the evaluation set; auxiliary data is external (July 2026, Chris).** The UW GT sample is small, so none of it is held out as a separate training/calibration release — teams tune on the external labelled datasets in RESOURCES.md (or self-transcribed UWDC pages) toward the released evaluation set. (An earlier draft of this repo described a second "calibration set" of UW pages; that was wrong and has been removed.)
...
- **Model constraint switched from <70B parameters to a 96 GB VRAM budget (July 2026, Chris).** The param cap was ambiguous (quantization, MoE total-vs-active parameters, per-model but unbounded ensembles) and didn't directly measure deployability. New rule: the submitted pipeline runs end-to-end on a single GPU with ≤96 GB VRAM — matching the campus RTX Pro 6000s — quantization allowed and encouraged, checked during the verification re-run. Submission card's `largest_model_params` field replaced by `peak_vram`.
