...
- **No leaderboard, no standings, self-scoring optional (July 2026, Chris).** Refines the hackathon decision: there is no automated metric run and no ranking of any kind — no standings post either. Writeups are reviewed as documented work; `score_local.py` stays as an optional self-assessment tool, CER fields on the submission card are optional, and any reported number must be reproducible from the posted code (re-run when a writeup is in contention for recognition).
