...
The auxiliary datasets in [RESOURCES.md](RESOURCES.md) — public labelled collections (Bentham, NARA RG 109, Alfred Escher, BLN600, IAM…) matched to the evaluation categories. The challenge's own UW ground truth is all in the evaluation set, which is for measuring, not training; if you want in-domain training pairs, the UW collections are publicly browsable and transcribing your own sample is encouraged. With modern pretrained VLMs you may not need to train at all — the released evaluation set tells you where you stand either way.
